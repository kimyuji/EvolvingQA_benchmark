...
def get_lr_scheduler(optimizer, total_num_steps, warmup_max_lr, warmup_num_steps):
    """
    Returns a WarmupDecayLR learning rate scheduler for PyTorch optimizer.
    """
    def lr_lambda(current_step):
        if current_step < warmup_num_steps:
            return float(current_step) / float(max(1, warmup_num_steps))
        else:
            return warmup_max_lr * math.exp(-math.log(total_num_steps - warmup_num_steps) * (current_step - warmup_num_steps) / (total_num_steps - warmup_num_steps))
...
class GPT2(pl.LightningModule):
    def __init__(self, hparams):
        ...
        if hparams.method=='baseline' or hparams.method=='mixreview':
            print(f"\nLoad pre-trained GPT2 baseline model from {hparams.model_name_or_path}...")
            self.model = GPT2LMHeadModel.from_pretrained(hparams.model_name_or_path)
        elif hparams.method=='kadapter':
            print(f"\nLoad pre-trained GPT2 K-adpater model from {hparams.model_name_or_path}...")
            self.model = GPT2_Kadapter.from_pretrained(hparams.model_name_or_path, ignore_mismatched_sizes=True)
            ...
            print(f"\nLoad pre-trained GPT2 LoRA model from {hparams.model_name_or_path}...")
            ...
            print(f"\nLoad pre-trained RecAdam model from {hparams.model_name_or_path}...")
            self.model = GPT2LMHeadModel.from_pretrained(hparams.model_name_or_path)
            self.pretrained_model = GPT2LMHeadModel.from_pretrained(hparams.model_name_or_path, ignore_mismatched_sizes=True)
        ...
        print(f"\n# Params : {calculate_parameters(self.model)}M")
        print(f"# Updated Params : {calculate_updated_parameters(self.model)}M")
    ...
    def _generative_step_finetune(self, batch, batch_idx):
        ...
        # self.log("clean_preds", str(clean_preds))
        # self.log("targets", str(targets))
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=-1):
        ...
        if self.hparams.mode == 'finetune_qa':
    ...
    def configure_optimizers(self, train_len=None):
        ...
        if self.hparams.method=='recadam':
            ...
            for n, p in new_model.named_parameters():
                if n == 'transformer.h.0.ln_1.weight':
                    print(p)
        ...
        if self.hparams.use_lr_scheduling:
            ...
            scheduler = get_lr_scheduler(optimizer, total_num_steps = total_num_steps, warmup_max_lr = self.hparams.learning_rate, warmup_num_steps = int(total_num_steps * 0.1))
