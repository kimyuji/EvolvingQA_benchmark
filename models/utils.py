...
def new_model_init(checkpoint, new_model):
    new_model_dict = new_model.state_dict()
    pretrained_dict = {}
    for k, v in checkpoint['state_dict'].items():
        _k = k.replace('model.transformer.', '')
        if _k in new_model_dict:
...
def calculate_parameters(model):
    keys = []
    return round(sum(p.numel() for n, p in model.named_parameters() if not any(key in n for key in keys))/1000000)

def calculate_updated_parameters(model):
    return round(sum(p.numel() for n, p in model.named_parameters() if p.requires_grad == True)/1000000)
